...
{ 
    ...
    public class GroupTrackerUpdater : MonoBehaviour
    {
        ...
        [Tooltip("Whether to add all the trackers of vehicles operated by non-player game agents in the scene.")]
        [SerializeField]
        protected bool addNonPlayerGameAgentsInScene = true;
        ...
        protected virtual void Awake()
        {
            ...
            {
                foreach(GameAgent gameAgent in GameAgentManager.Instance.GameAgents)
                {
                    OnGameAgentRegistered(gameAgent);
                }

                GameAgentManager.Instance.onGameAgentRegistered += OnGameAgentRegistered;
            }

            foreach (GameAgent gameAgent in gameAgents)
            {
                AddGameAgent(gameAgent);
            }
        ...
        protected virtual void OnGameAgentRegistered(GameAgent gameAgent)
        {
            if (addNonPlayerGameAgentsInScene && !gameAgent.IsPlayer && gameAgents.IndexOf(gameAgent) == -1)
            {
                AddGameAgent(gameAgent);
            }
        }
        ...
        protected virtual void AddGameAgent(GameAgent gameAgent)
        {
