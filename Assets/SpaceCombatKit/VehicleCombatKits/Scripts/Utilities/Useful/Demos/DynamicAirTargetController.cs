...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class DynamicAirTargetController : MonoBehaviour
    {
        ...
        protected void Awake()
        {
            ...
            rBody.linearVelocity = Vector3.right * startingDirection * Random.Range(minSpeed, maxSpeed);
        }
        ...
        protected void Update()
        {
            ...
            if (transform.localPosition.x < minX && rBody.linearVelocity.x < 0)
            {
                velocity = Vector3.right * Random.Range(minSpeed, maxSpeed);
            }
            else if (transform.localPosition.x > maxX && rBody.linearVelocity.x > 0)
            {
                velocity = -Vector3.right * Random.Range(minSpeed, maxSpeed);
            }
