...
namespace VSX.Characters
{
    public class RigidbodyCharacterFootstepAudio : MonoBehaviour
    {
        ...
        protected virtual void OnFootstep()
        {
            ...
            float walkAmount = Mathf.Min((m_RigidbodyCharacterController.Rigidbody.linearVelocity.magnitude / m_RigidbodyCharacterController.WalkSpeed), 1);

            float runMargin = m_RigidbodyCharacterController.RunSpeed - m_RigidbodyCharacterController.WalkSpeed;
            float runAmount = (runMargin == 0 || m_RigidbodyCharacterController.Reversing) ? 0 : Mathf.Max(m_RigidbodyCharacterController.Rigidbody.linearVelocity.magnitude - m_RigidbodyCharacterController.WalkSpeed, 0) / runMargin;
