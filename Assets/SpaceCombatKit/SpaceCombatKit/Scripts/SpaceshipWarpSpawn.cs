        ...
        void FinishAnimation(float finishAmount)
        {
            if (enablePhysicsOnRelease)
            {
                ...
                if (applyVelocityOnFinish)
                {
                    ...
                    spawnedVehicle.CachedRigidbody.linearVelocity = (spawnedVehicle.CachedRigidbody.position - previousPosition) / Time.fixedDeltaTime;
                }
                else
                {
                    spawnedVehicle.CachedRigidbody.linearVelocity = Vector3.zero;
