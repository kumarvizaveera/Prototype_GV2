...
namespace VSX.SpaceCombatKit
{
    ...
    public class ObstacleData
	{
		...
		public ObstacleData(RaycastHit hit, float detectionTime, Rigidbody rigidBody)
		{
			...
			if (rigidBody != null)
			{
				...
				this.obstacleVelocity = rigidBody.linearVelocity;
			} 
			else 
			{
				this.rigidBody = null;
	...
	public class ObstacleAvoidanceBehaviour : AISpaceshipBehaviour
	{
        ...
        void UpdateRiskFactor(ObstacleData obstacleData, bool show)
        {
            ...
            Vector3 collisionRelVelocity = obstacleData.obstacleVelocity - vehicle.CachedRigidbody.linearVelocity;
            ...
            Vector3 directionReference = vehicle.CachedRigidbody.linearVelocity.normalized;
            if (vehicle.CachedRigidbody.linearVelocity.magnitude < 0.0001f)
            {
		...
		public override bool BehaviourUpdate()
		{
            ...
            float obstacleScanDistance = includeTimeToImpactFactor ? vehicle.CachedRigidbody.linearVelocity.magnitude * zeroRiskTimeToImpact : defaultObstacleScanDistance;
            ...
            if (nextSphereCastIndex != -1)
            {
                ...
                Vector3 projectionVector = vehicle.CachedRigidbody.linearVelocity.normalized;
                if (vehicle.CachedRigidbody.linearVelocity.magnitude < 0.0001f)
                {
