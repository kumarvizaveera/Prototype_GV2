...
namespace VSX.SpaceCombatKit
{
    ...
    public class FlybyParticleController : MonoBehaviour 
	{
        ...
        public virtual void UpdateEffect(Vehicle vehicle)
		{
            ...
            {
                size = Mathf.Clamp(vehicle.CachedRigidbody.linearVelocity.magnitude * velocityToSizeCoefficient, 0, maxSize);
            }
            ...
            {
                lengthScale = Mathf.Clamp(vehicle.CachedRigidbody.linearVelocity.magnitude * velocityToLengthScaleCoefficient, 0, maxLengthScale);
            }
            ...
            if (velocityBasedAlpha)
            {
                alpha = Mathf.Clamp(vehicle.CachedRigidbody.linearVelocity.magnitude * velocityToAlphaCoefficient, 0, maxAlpha);
            }
            ...
            velocityOverLifetimeModule.x = -vehicle.CachedRigidbody.linearVelocity.x;
            velocityOverLifetimeModule.y = -vehicle.CachedRigidbody.linearVelocity.y;
            velocityOverLifetimeModule.z = -vehicle.CachedRigidbody.linearVelocity.z;

            
            if (vehicle.CachedRigidbody.linearVelocity != Vector3.zero)
            {
                transform.rotation = Quaternion.LookRotation(vehicle.CachedRigidbody.linearVelocity.normalized, vehicle.transform.up);
                transform.position = vehicle.CachedRigidbody.transform.position + vehicle.transform.forward * frontOffset + vehicle.CachedRigidbody.linearVelocity.normalized * leadDistance;
            }
