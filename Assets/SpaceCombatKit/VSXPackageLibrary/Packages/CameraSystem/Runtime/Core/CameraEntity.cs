...
namespace VSX.CameraSystem
{
    ...
    public class CameraEntity : MonoBehaviour
    {
        ...
        protected WaitForFixedUpdate cameraCollisionFixedUpdateYield = new WaitForFixedUpdate();
        ...
        protected IEnumerator CameraCollisionUpdateCoroutine()
        {
            while (true)
            {
                yield return cameraCollisionFixedUpdateYield;  // Wait until the camera controller fixed update is finished

                if (cameraTarget != null && cameraTarget.Rigidbody != null) CameraCollisionUpdate();
            }
        }
