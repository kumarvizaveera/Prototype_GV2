            ...
            if (data.Slots.Count > 0)
            {
                ...
                for (int i = 0; i < data.Slots.Count; ++i)
                {
                    ...
                    for (int j = 0; j < data.Slots[i].selectedModules.Count; ++j)
                    {
                        ...
                        if (!loadoutItems.vehicles[data.Slots[i].selectedVehicleIndex].vehiclePrefab.ModuleMounts[j].IsCompatible(loadoutItems.modules[data.Slots[i].selectedModules[j]].modulePrefab))
                        {
                            data.Slots[i].selectedModules[j] = -1;
                            continue;
                        }
